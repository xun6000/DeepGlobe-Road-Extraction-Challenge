...
def default_loader(id, root):
    ...
    a = [img[:]]
    b= [mask[:]]
    c = randomHueSaturationValue(img,
                                   hue_shift_limit=(-30, 30),
                                   sat_shift_limit=(-5, 5),
                                   val_shift_limit=(-15, 15))
    a.append(c[:])
    b.append(b[0][:])
    d, e = randomShiftScaleRotate(img, mask,
                                       shift_limit=(-0.1, 0.1),
                                       scale_limit=(-0.1, 0.1),
    ...
    d, e = randomHorizontalFlip(img, mask)
    a.append(d[:])
    b.append(e[:])

    d, e = randomVerticleFlip(img, mask)
    a.append(d[:])
    b.append(e[:])
    d, e= randomRotate90(img, mask)
    a.append(d[:])
    b.append(e[:])
    b = np.expand_dims(b, axis=3)

    a = np.array(a, np.float32).transpose(0,3,1,2)/255.0 * 3.2 - 1.6
    b = np.array(b, np.float32).transpose(3,0,1,2)/255.0
    b[b>=0.5] = 1
    b[b<=0.5] = 0
